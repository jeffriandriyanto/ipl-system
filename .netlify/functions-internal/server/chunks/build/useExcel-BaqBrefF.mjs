import * as cpexcel from '/Users/exiglosi/Documents/Jeffri/ipl-system/node_modules/xlsx/dist/cpexcel.js';
...
//#region app/composables/useExcel.ts
var useExcel = () => {
	/**
	...
	*/
	function exportToExcel(data, filename, sheetName = "Sheet1") {
		...
		const ws = xlsxExports.utils.json_to_sheet(data);
		ws["!cols"] = Object.keys(data[0] || {}).map((key) => ({ wch: Math.max(key.length, ...data.map((row) => String(row[key] || "").length)) + 2 }));
		xlsxExports.utils.book_append_sheet(wb, ws, sheetName);
		xlsxExports.writeFile(wb, `${filename}.xlsx`);
	}
	/**
	* Export multiple sheets to Excel
	*/
	function exportMultiSheetToExcel(sheets, filename) {
		const wb = xlsxExports.utils.book_new();
		for (const sheet of sheets) {
			const ws = xlsxExports.utils.json_to_sheet(sheet.data);
			ws["!cols"] = Object.keys(sheet.data[0] || {}).map((key) => ({ wch: Math.max(key.length, ...sheet.data.map((row) => String(row[key] || "").length)) + 2 }));
			xlsxExports.utils.book_append_sheet(wb, ws, sheet.name);
		}
		xlsxExports.writeFile(wb, `${filename}.xlsx`);
	}
	/**
		...
		return new Promise((resolve, reject) => {
			...
			reader.onload = (e) => {
				try {
					...
					resolve(xlsxExports.utils.sheet_to_json(firstSheet));
				} catch (error) {
					reject(error);
				}
			};
	...
	*/
	function downloadTemplate(headers, filename, sampleData = []) {
		const wb = xlsxExports.utils.book_new();
		const headerRow = headers.map((h) => h.label);
		const ws = xlsxExports.utils.aoa_to_sheet([headerRow, ...sampleData.map((row) => headers.map((h) => row[h.key] || ""))]);
		ws["!cols"] = headers.map((h) => ({ wch: Math.max(h.label.length, ...sampleData.map((row) => String(row[h.key] || "").length)) + 2 }));
		xlsxExports.utils.book_append_sheet(wb, ws, "Template");
		xlsxExports.writeFile(wb, `template-${filename}.xlsx`);
	}
	/**
	* Format Rupiah for Excel
	*/
	function formatRupiahExcel(amount) {
...
export { useExcel as u };
//# sourceMappingURL=useExcel-BaqBrefF.mjs.map
