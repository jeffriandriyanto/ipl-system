 ...
 */
export function exportMultiSheetToExcel(sheets: { name: string; data: any[] }[], filename: string) {
  const wb = XLSX.utils.book_new()
 ...
 */
export function downloadTemplate(headers: { key: string; label: string }[], filename: string, sampleData: any[] = []) {
  const wb = XLSX.utils.book_new()
